...
class Dragon {
    ...
    draw(ctx) {
        if (this.spritesheet.complete && this.spritesheet.naturalWidth !== 0) {
            ...
            ctx.rotate(rotation);
            // Default check to prevent error if frameWidth is invalid
            if (this.frameWidth > 0) {
...
class Enemy {
    ...
    update(game) {
        ...
        if (this.type !== 'scout') {
            this.shootTimer++;
            if (this.shootTimer > 150 && Math.random() < 0.05) {
...
class Game {
    constructor() {
        ...
        if (this.ui.shootBtn) {
            this.ui.shootBtn.addEventListener('click', () => this.shoot());
            this.ui.shootBtn.addEventListener('touchstart', (e) => { e.preventDefault(); this.shoot(); }, {passive: false});
        }
    ...
    update() {
        ...
        if (this.bgX < -window.innerWidth) this.bgX = 0;
        ...
        for (let i = this.obstacles.length - 1; i >= 0; i--) {
            ...
            if (!o.passed && this.dragon.x > o.x + o.width) {
                ...
                if(this.score > 0 && this.score % 50 === 0) this.startBossBattle();
            }
        }
    ...
    draw() {
        try {
            ...
            // Draw BG or Color Fallback
            if (this.bgImage && this.bgImage.complete && this.bgImage.naturalWidth > 0) {
                this.ctx.drawImage(this.bgImage, this.bgX, 0, window.innerWidth, window.innerHeight);
                this.ctx.drawImage(this.bgImage, this.bgX + window.innerWidth, 0, window.innerWidth, window.innerHeight);
            } else {
